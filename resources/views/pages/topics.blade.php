@extends('layouts.app')
@section('title', text('topics.page_title'))
        ...
        <main class="lg:col-span-8 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <form method="GET" action="{{ route('topics') }}" class="search-input-wrapper w-full sm:max-w-md">
                    ...
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="{{ text('topics.search_placeholder') }}" class="bg-transparent border-none text-xs sm:text-sm font-bold text-[rgb(var(--on-surface))] placeholder-[rgb(var(--on-surface))/0.4] focus:outline-none w-full" data-auto-search />
                    @if($search)
                ...
                <div class="text-3xs font-black uppercase tracking-widest text-[rgb(var(--on-surface))/0.5]">
                    {{ text('topics.showing', ['count' => $topics->count(), 'total' => $topics->total()]) }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <x-card
                    ...
                    badgeText="{{ text('topics.card_badge', ['num' => str_pad($topics->firstItem() + $i, 2, '0', STR_PAD_LEFT)]) }}"
                    title="{{ $topic->name }}"
                    description="{{ text('topics.card_desc') }}"
                    progress="0"
                ...
                <div class="col-span-full bg-[rgb(var(--surface-container-lowest))] border-2 border-dashed border-[rgb(var(--surface-container-high))] rounded-3xl p-12 text-center space-y-3">
                    ...
                    <h4 class="font-black text-base text-[rgb(var(--on-surface))] uppercase">{{ text('topics.empty_title') }}</h4>
                    <p class="text-xs font-semibold text-[rgb(var(--on-surface))/0.5] max-w-sm mx-auto">
                        {{ $search ? text('topics.empty_search') : text('topics.empty_no_query') }}
                    </p>
                </div>
                @endforelse
            </div>
        ...
        <aside class="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            ...
            <div class="sidebar-widget-card space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-black text-sm uppercase tracking-wide text-[rgb(var(--on-surface))]">{{ text('topics.sidebar_level') }}</h4>
                        <p class="text-3xs font-bold text-[rgb(var(--on-surface))/0.5]">{{ text('topics.sidebar_level_desc') }}</p>
                    </div>
                </div>
                ...
                <div class="bg-[rgb(var(--surface-container-high))/0.5] rounded-2xl p-3.5 flex items-center justify-between">
                    <div>
                        <span class="text-3xs font-black uppercase tracking-widest text-[rgb(var(--primary))]">{{ text('topics.sidebar_level_label', ['level' => $level]) }}</span>
                        <div class="text-base font-black text-[rgb(var(--on-surface))]">{{ text('topics.sidebar_stars', ['count' => $totalStars]) }}</div>
                    </div>
                ...
                <a href="{{ route('login') }}" class="w-full py-3 bg-[rgb(var(--primary))] text-white rounded-full font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 no-underline shadow-lg shadow-[rgb(var(--primary))/0.2]">
                    <span>{{ text('topics.sidebar_login') }}</span>
            ...
            <div class="sidebar-widget-card bg-gradient-to-br from-indigo-600 to-violet-700 text-white border-none space-y-4 shadow-xl shadow-indigo-500/20">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="font-black text-base uppercase tracking-tight">{{ text('topics.sidebar_exam_title') }}</h4>
                        <p class="text-3xs font-semibold text-white/80">{{ text('topics.sidebar_exam_desc') }}</p>
                    </div>
                </div>
                <a href="{{ route('exam') }}" class="w-full py-3 bg-white text-indigo-950 rounded-full font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 no-underline active:scale-95 transition-all shadow-md">
                    <span>{{ text('topics.sidebar_exam_btn') }}</span>
                    <span class="material-symbols-outlined !text-lg">arrow_forward</span>
                </a>
            </div>
            ...
            <div class="sidebar-widget-card space-y-2.5">
                <div class="flex items-center gap-2 text-amber-600">
                    ...
                    <h4 class="font-black text-xs uppercase tracking-wide">{{ text('topics.sidebar_tip_title') }}</h4>
                </div>
                <p class="text-xs font-semibold text-[rgb(var(--on-surface))/0.6] leading-relaxed">
                    {{ text('topics.sidebar_tip_desc') }}
                </p>
            </div>
