@extends('layouts.app')
@section('title', text('profile.page_title'))
...
<section class="max-w-6xl mx-auto px-4 py-8 md:py-12 space-y-6 md:space-y-0 md:grid md:grid-cols-2 md:gap-6">
    ...
    <div class="profile-header md:col-span-2">
        ...
        <div class="profile-header__content">
            <div class="profile-avatar relative group">
                ...
                <button type="button" onclick="openAvatarModal()" class="profile-avatar__edit shadow-lg transition-transform hover:scale-110 bg-[rgb(var(--primary))] text-white border-2 border-white" title="{{ text('profile.avatar_edit_title') }}">
                    <span class="material-symbols-outlined !text-base">photo_camera</span>
                </button>
            </div>
            <h1 class="profile-header__name">{{ $profile['name'] ?? text('profile.name_fallback') }}</h1>
            <p class="profile-header__email">{{ $profile['email'] ?? text('profile.email_fallback') }}</p>
            <div class="profile-header__stars">
                ...
                <span>{{ $totalStars ?? 0 }} {{ text('profile.header_stars') }}</span>
            </div>
            <div class="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-white/10">
                ...
                <button class="profile-theme profile-theme--default" data-theme="default" title="{{ text('profile.theme_default') }}">
                    <span class="material-symbols-outlined !text-sm">check</span>
                </button>
                <button class="profile-theme profile-theme--pink" data-theme="pink" title="{{ text('profile.theme_pink') }}"></button>
                <button class="profile-theme profile-theme--yellow" data-theme="yellow" title="{{ text('profile.theme_yellow') }}"></button>
                <button class="profile-theme profile-theme--green" data-theme="green" title="{{ text('profile.theme_green') }}"></button>
                <button class="profile-theme profile-theme--red" data-theme="red" title="{{ text('profile.theme_red') }}"></button>
            </div>
        </div>
    </div>
    ...
    <div class="profile-section">
        <div class="profile-section__header">
            ...
            <div>
                <h3 class="profile-section__title">{{ text('profile.info_title') }}</h3>
                <p class="profile-section__desc">{{ text('profile.info_desc') }}</p>
            </div>
        </div>
        <form method="POST" action="{{ route('profile.update') }}" id="profileForm">
            ...
            <div class="profile-form-grid">
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_name') }}</label>
                    <div class="profile-field__input-wrap">
                        <span class="material-symbols-outlined !text-lg text-[rgb(var(--on-surface))/0.3]">person</span>
                        <input type="text" name="name" value="{{ $profile['name'] ?? '' }}" class="profile-field__input" placeholder="{{ text('profile.placeholder_name') }}" />
                    </div>
                </div>
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_email') }}</label>
                    <div class="profile-field__input-wrap">
                        <span class="material-symbols-outlined !text-lg text-[rgb(var(--on-surface))/0.3]">mail</span>
                        <input type="email" name="email" value="{{ $profile['email'] ?? '' }}" class="profile-field__input" placeholder="{{ text('profile.placeholder_email') }}" />
                    </div>
                </div>
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_phone') }}</label>
                    <div class="profile-field__input-wrap">
                        <span class="material-symbols-outlined !text-lg text-[rgb(var(--on-surface))/0.3]">call</span>
                        <input type="tel" name="phone" value="{{ $profile['phone'] ?? '' }}" class="profile-field__input" placeholder="{{ text('profile.placeholder_phone') }}" />
                    </div>
                </div>
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_school') }}</label>
                    <div class="profile-field__input-wrap">
                        <span class="material-symbols-outlined !text-lg text-[rgb(var(--on-surface))/0.3]">school</span>
                        <input type="text" name="school_name" value="{{ $student->school_name ?? '' }}" class="profile-field__input" placeholder="{{ text('profile.placeholder_school') }}" />
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="profile-field">
                        <label class="profile-field__label">{{ text('profile.label_city') }}</label>
                        <div class="profile-field__input-wrap">
                            ...
                            <select name="city_id" class="profile-field__input profile-field__select">
                                <option value="">{{ text('profile.placeholder_city') }}</option>
                    ...
                    <div class="profile-field">
                        <label class="profile-field__label">{{ text('profile.label_grade') }}</label>
                        <div class="profile-field__input-wrap">
                            ...
                            <select name="grade_id" class="profile-field__input profile-field__select">
                                <option value="">{{ text('profile.placeholder_grade') }}</option>
                ...
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_birth') }}</label>
                    <div class="profile-field__input-wrap">
    ...
    <div class="profile-section">
        <div class="profile-section__header">
            ...
            <div>
                <h3 class="profile-section__title">{{ text('profile.password_title') }}</h3>
                <p class="profile-section__desc">{{ text('profile.password_desc') }}</p>
            </div>
        </div>
        <form method="POST" action="{{ route('profile.password') }}">
            ...
            <div class="profile-form-grid">
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_current_pass') }}</label>
                    <div class="profile-field__input-wrap">
                ...
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_new_pass') }}</label>
                    <div class="profile-field__input-wrap">
                ...
                <div class="profile-field">
                    <label class="profile-field__label">{{ text('profile.label_confirm_pass') }}</label>
                    <div class="profile-field__input-wrap">
    ...
    <div class="profile-section md:col-span-2">
        <div class="profile-section__header">
            ...
            <div>
                <h3 class="profile-section__title">{{ text('profile.achievements_title') }}</h3>
                <p class="profile-section__desc">{{ text('profile.achievements_desc') }}</p>
            </div>
        </div>
    ...
    <div class="w-full max-w-md rounded-3xl bg-[rgb(var(--surface-container-lowest))] shadow-2xl border border-[rgb(var(--surface-container-high))] overflow-hidden flex flex-col max-h-[90vh] text-[rgb(var(--on-surface))]" style="background-color: rgb(var(--surface-container-lowest)) !important;">
        ...
        <div class="flex items-center justify-between border-b border-[rgb(var(--surface-container-high))] bg-[rgb(var(--surface-container))] px-6 py-4 shrink-0">
            <div class="flex items-center gap-2.5">
                ...
                <h3 class="text-base font-black text-[rgb(var(--on-surface))]">{{ text('profile.avatar_upload_title') }}</h3>
            </div>
        ...
        <div class="p-6 space-y-6 overflow-y-auto max-h-[75vh]">
            ...
            <form method="POST" action="{{ route('profile.avatar') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex flex-col items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-bold text-[rgb(var(--on-surface-variant))] uppercase tracking-wider">{{ text('profile.avatar_preview') }}</span>
                </div>

                <!-- Input -->
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-[rgb(var(--on-surface))]">{{ text('profile.avatar_select') }}</label>
                    <input 
                        type="file" 
                        name="avatar_file" 
                    ...
                    />
                    <p class="text-[11px] text-[rgb(var(--on-surface-variant))/0.7] font-medium">{{ text('profile.avatar_compress') }}</p>
                </div>
